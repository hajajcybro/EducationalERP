...
class UniversityApplication(models.Model):
    ...
    def _compute_display_name(self):
        for rec in self:
            if rec.last_name and rec.middle_name:
                rec.display_name = f"{rec.name} {rec.middle_name} {rec.last_name}"
            elif rec.middle_name and not rec.last_name:
                rec.display_name = f"{rec.name} {rec.middle_name}"
            elif rec.last_name and not rec.middle_name:
    ...
    def create(self, vals):
        ...
        return super(UniversityApplication, self).create(vals)

    @api.onchange('state_id', 'per_state_id')
    def _onchange_state(self):
        if self.state_id.country_id:
            self.country_id = self.state_id.country_id
        if self.per_state_id.country_id:
    ...
    def _onchange_date_of_birth(self):
        ...
        if self.date_of_birth:
            if (fields.date.today().year - self.date_of_birth.year) < 18:
                raise ValidationError(_('Please provide a valid Date of Birth'))
