...
class AccountMove(models.Model):
    ...
    fee_structure_ids = fields.Many2many('fee.structure',
                                         ...
                                         help="Select a fee structure", )
    is_fee = fields.Boolean(string='Is Fee',
                            help="To determine whether the journal entry"
                                 " is for fee or not")
    ...
    journal_id = fields.Many2one(related='fee_category_id.journal_id',
                                 store=True, help="Journal of the receipt")
    ...
    def create(self, vals):
        ...
        if vals.get('fee_category_id'):
            vals.update({
                'is_fee': True,
                'student_name': self.env['res.partner'].browse(
                    vals.get('partner_id')).name
            })
