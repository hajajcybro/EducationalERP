...
#    Author: Anupriya Ashok @cybrosys(odoo@cybrosys.com)
#
...
class InsuranceDetails(models.Model):
    ...
    policy_id = fields.Many2one('policy.details', string='Policy',
                                required=True, help="Select the policy details and the policy preferred")
    ...
    policy_duration = fields.Integer(string='Duration in months', required=True,
                                     help="Specify the policy duration to which this policy exists")
    ...
    def action_create_invoice(self):
        ...
        total_invoiced = sum(self.invoice_ids.mapped('amount_total'))
        remaining = self.amount - total_invoiced

        if remaining <= 0 or remaining < 0.01:
            self.hide_inv_button = True
            raise ValidationError(_("All installments have already been invoiced."))

        # ---- FIX 2: Last installment should not exceed remaining amount ----
        if self.payment_type == 'fixed':
            price = self.amount
            self.hide_inv_button = False
        else:
            price = min(self.amount_installment, remaining)
