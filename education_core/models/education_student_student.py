...
class EducationStudentStudent(models.Model):
    ...
    def _compute_age(self):
        ...
        class EducationStudentStudent(models.Model):
            ...
            state_id = fields.Many2one('res.country.state', 'Fed. State', domain="[('country_id', '=?', country_id)]",
                                       help='Select the state or province.')
            country_code = fields.Char(
                related='country_id.code',
                string='Country Code',
                readonly=True,
            ...
            def create(self, vals_list):
                ...
                for vals in vals_list:
                    if vals.get('reference_no', _('New')) == _('New'):
                        vals['reference_no'] = self.env['ir.sequence'].next_by_code('education_student_temporary') or _(
                            'New')
                ...
                for record in self:
                    if record.email and not re.match(email_pattern, record.email):
                        raise ValidationError(
                            _('Invalid email address. Please enter a valid format like name@example.com.'))
                    if record.phone and not re.match(phone_pattern, record.phone):
                        raise ValidationError(
                            _('Invalid phone number. Please enter digits only, 7–15 numbers, with optional +.'))
    ...
    @api.model_create_multi
    def create(self, vals_list):
        """Create student and automatically create related user."""
        print('create user')
        students = super().create(vals_list)
        # student_group = self.env.ref('education_core.group_education_student')
        for student in students:
            if not student.user_id:
                user_vals = {
                    'name': student.name,
                    'login': student.email,
                    'groups_id' : [fields.Command.link(self.env.ref('education_core.group_education_student').id)]
                }
