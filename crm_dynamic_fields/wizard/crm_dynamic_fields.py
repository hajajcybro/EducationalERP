...
class CRMDynamicFields(models.TransientModel):
    ...
    def write(self, vals):
        ...
        for record in self:
            if 'field_type' in vals and vals['field_type'] != record.field_type:
                record.unlink()
                for key, value in vals.items():
                    setattr(record, key, value)
                record.action_create_fields()
            else:
                ...
                if base_field:
                    base_field.write({
                        ...
                        'translate': record.is_translate,
                        'selection': record.selection_field,
                    })
                    ...
                    #
                    # if update_vals:
                    #     base_field.write({'ttype': vals['field_type']})
