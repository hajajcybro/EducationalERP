...
class ImportWizard(models.TransientModel):
    ...
    def copy_import(self, res_id, model, columns):
        ...
        try:
            ...
            for item in columns:
                ...
                if '/' in field_path:
                    ...
                    if field_info['type'] != 'one2many':
                        column_mapping[excel_column_name] = parent_field
            ...
            for field, default_val in defaults.items():
                if field not in data.columns and field not in missing_required and field in model_fields:
                    # Skip complex relational defaults, handle only simple and many2one fields
                    if model_fields[field]['type'] not in ['one2many', 'many2many']:
                        data = data.copy()
                        data.loc[:, field] = [default_val] * len(data)
                        imported_fields.add(field)
            t_m2o = time.time()
            many2one_fields = {}
    ...
    def _postgres_bulk_import_fast(self, data, model, final_fields, m2m_trigger_val, o2m_trigger_val,
        ...
        try:
            Model = self.env[model]
            inserted_row_ids = {}
            self.env.cr.execute(f"SELECT column_name FROM information_schema.columns WHERE table_name = '{table_name}'")
            # db_columns = [row[0] for row in self.env.cr.fetchall()]
            # --- FIX: Account.Account Specific Logic ---
            db_cols_info = self.env.cr.fetchall()
            db_columns = [row[0] for row in db_cols_info]
            db_column_types = {row[0]: row[1] for row in db_cols_info}
            if model == 'account.account':
                # 1. Force create_asset (Fixes NOT NULL constraint error)
                if 'create_asset' in db_columns and 'create_asset' not in final_fields:
                    data = data.copy()
                    data.loc[:, 'create_asset'] = [False] * len(data)
                    final_fields.append('create_asset')
                    # IMPORTANT: Add to model_fields so the data formatter processes it as boolean
                    if 'create_asset' not in model_fields:
                        model_fields['create_asset'] = {'type': 'boolean', 'store': True}
                    _logger.info("[AUTO-FILL] Set create_asset=False for account.account")

                # 2. Map Account Type (Text -> Key)
                if 'account_type' in data.columns:
                    try:
                        selection = self.env['account.account'].fields_get(['account_type'])['account_type'][
                            'selection']
                        type_map = {}
                        for key, label in selection:
                            type_map[str(label)] = key
                            type_map[str(label).lower()] = key
                            type_map[str(key)] = key

                        def map_account_type(val):
                            if pd.isna(val) or val is None or str(val).strip() == "":
                                return None
                            val_str = str(val).strip()
                            return type_map.get(val_str, type_map.get(val_str.lower(), val_str))

                        data = data.copy()
                        data['account_type'] = data['account_type'].apply(map_account_type)
                    except Exception as e:
                        _logger.warning(f"Could not map account_type: {e}")

            if 'code' in data.columns and 'code_store' in db_columns:
                company_id_str = str(self.env.company.id)

                def build_code_store(code_val):
                    if pd.isna(code_val) or code_val is None:
                        return None
                    code_str = str(code_val).strip()
                    if code_str in ('', 'nan', 'None', 'false'):
                        return None
                    # Remove ".0" suffix added by Excel/pandas for numeric codes
                    if code_str.endswith('.0'):
                        code_str = code_str[:-2]
                    return json.dumps({company_id_str: code_str}, ensure_ascii=False)

                data = data.copy()
                data.loc[:, 'code_store'] = data['code'].apply(build_code_store)
                if 'code_store' not in final_fields:
                    final_fields.append('code_store')
                _logger.info("[AUTO-FILL] Built code_store as {%s: code} for %d records",
                             company_id_str, len(data))
            # --- END FIX ---
            ...
            # =========================================================
            if model == 'account.account' and 'create_asset' in model_fields:
                if 'create_asset' not in final_fields:
                    data = data.copy()
                    data.loc[:, 'create_asset'] = [False] * len(data)
                    final_fields.append('create_asset')
                    _logger.info("[AUTO-FILL] Set create_asset=False for account.account")
            ...
            for _col in list(data.columns):
                if _col not in model_fields:
                    continue
                _finfo = model_fields[_col]
                if _finfo.get('type') == 'selection' and _finfo.get('selection'):
                    _raw_sel = _finfo['selection']
                    # fields_get returns list of [key, label] or (key, label)
                    _sel_map = {}
                    for _item in _raw_sel:
                        _key, _label = _item[0], _item[1]
                        _sel_map[str(_key).strip().lower()] = _key  # key passthrough
                        _sel_map[str(_label).strip().lower()] = _key  # label -> key

                    def _convert_sel(val, m=_sel_map):
                        if val is None or (isinstance(val, float) and pd.isna(val)) or str(val).strip() == '':
                            return val
                        return m.get(str(val).strip().lower(), str(val).strip())

                    data[_col] = data[_col].apply(_convert_sel)
                    _logger.info("[FIX] selection field '%s' labels converted to keys", _col)

            default_lang = self.env.context.get('lang') or getattr(self.env, 'lang', None) or 'en_US'

            if model == 'account.account':
                self.env.cr.execute(
                    "SELECT column_name FROM information_schema.columns WHERE table_name = %s AND column_name = 'code_store'",
                    (table_name,)
                )
                if self.env.cr.fetchone() and 'code' in data.columns and 'code_store' not in final_fields:
                    company_id_str = str(self.env.company.id)

                    def _build_code_store_fast(val):
                    ...
                    data = data.copy()
                    data['code_store'] = data['code'].apply(_build_code_store_fast)
                    final_fields.append('code_store')
                    _logger.info("[FIX] code_store populated for %d account.account records", len(data))
            ...
            for column in data.columns:
                if column in model_fields:
                    ...
                    if field_info.get('translate') and field_info.get('store') and column != 'code_store':
            ...
            for column in processed_data.columns:
                if column in model_fields and column not in translatable_columns:
                    ...
                    if field_type in ['char', 'text']:
                        ...
                        if model == 'account.account' and column == 'code':
                            # This removes the ".0" if Excel sent "101000.0"
                            processed_data.loc[:, column] = processed_data[column].apply(
                        ...
                        # Convert display label -> internal key before DB insert
                        # e.g. 'Expenses' -> 'expense_other', 'Current Assets' -> 'asset_current'
                        raw_selection = field_info.get('selection', [])
                        sel_map = {}
                        for key, label in raw_selection:
                            sel_map[str(key).strip().lower()] = key  # key -> key (passthrough)
                            sel_map[str(label).strip().lower()] = key  # label -> key

                        def _map_selection(val, _sel_map=sel_map):
                            if val is None or (isinstance(val, float) and pd.isna(val)) or str(val).strip() == '':
                                return None
                            val_lower = str(val).strip().lower()
                            return _sel_map.get(val_lower, str(val).strip())

                        processed_data.loc[:, column] = processed_data[column].apply(_map_selection)
            csv_buffer = BytesIO()
            ...
            for column in data_for_copy.columns:
                db_type = db_column_types.get(column, '')
                is_json_db = db_type in ['json', 'jsonb']
                if is_json_db:
                    # FIX: Postgres strictly requires JSON formatting for json/jsonb columns.
                    # Plain text like "optional" will cause a syntax error unless wrapped in quotes.
                    json_vals = []
                    for val in data_for_copy[column]:
                        if val is None or pd.isna(val) or val == '':
                            ...
                            # If it's already a formatted JSON object or array, leave it as is
                            if (str_val.startswith('{') and str_val.endswith('}')) or (
                                    str_val.startswith('[') and str_val.endswith(']')):
                                json_vals.append(str_val)
                            else:
                                # Wrap plain text in valid JSON quotes (e.g., "optional" -> "\"optional\"")
                                json_vals.append(json.dumps(val, ensure_ascii=False))
                    data_for_copy.loc[:, column] = json_vals
    ...
    def _postgres_bulk_import_enhanced(self, data, model, final_fields, m2m_trigger_val, o2m_trigger_val,
        ...
        try:
            ...
            if model == 'account.account' and 'create_asset' in existing_columns and 'create_asset' not in available_columns:
                insert_data = insert_data.copy()
                insert_data['create_asset'] = [False] * len(insert_data)
                available_columns.append('create_asset')
                _logger.info("[AUTO-FILL] Set create_asset=False for account.account (Enhanced Mode)")

            if model == 'account.account' and 'code_store' in existing_columns:
                company_id_str = str(self.env.company.id)
                # Get 'code' from insert_data if present, else from original data
                code_series = None
                if 'code' in insert_data.columns:
                    code_series = insert_data['code']
                elif 'code' in data.columns:
                    code_series = data['code']
                if code_series is not None:
                    def _build_code_store(val):
                        if val is None or (isinstance(val, float) and pd.isna(val)):
                            return None
                        code_str = str(val).strip()
                        if code_str in ('', 'nan', 'None', 'false'):
                    ...
                    insert_data = insert_data.copy()
                    insert_data['code_store'] = code_series.apply(_build_code_store)
                    if 'code_store' not in available_columns:
                        available_columns.append('code_store')
                    _logger.info("[AUTO-FILL] Built code_store as {%s: code} for account.account (Enhanced Mode)",
                                 company_id_str)
            # FIX: Handle account.account Constraints & Type Mapping


            # FIX: Handle account.account Constraints & Type Mapping
            # =========================================================
            # if model == 'account.account':
            #     # Fix: create_asset cannot be Null
            #     if 'create_asset' in existing_columns:
            #         if 'create_asset' not in insert_data.columns:
            #             insert_data['create_asset'] = False
            #             available_columns.append('create_asset')
            #         else:
            #             insert_data['create_asset'] = insert_data['create_asset'].fillna(False)
            #
            #     # Fix: reconcile cannot be Null
            #     if 'reconcile' in existing_columns:
            #         if 'reconcile' not in insert_data.columns:
            #             insert_data['reconcile'] = False
            #             available_columns.append('reconcile')
            #         else:
            #             insert_data['reconcile'] = insert_data['reconcile'].fillna(False)
            #
            #     # Fix: Clean Account Code (remove .0)
            #     if 'code' in insert_data.columns:
            #         insert_data['code'] = insert_data['code'].astype(str).apply(
            #             lambda x: x.replace('.0', '') if x.endswith('.0') else x
            #         )
            #
            #     # Fix: Map Account Type
            #     if 'account_type' in insert_data.columns:
            #         try:
            #             selection = env['account.account'].fields_get(['account_type'])['account_type']['selection']
            #             type_map = {}
            #             for key, label in selection:
            #                 type_map[str(label).strip()] = key
            #                 type_map[str(label).strip().lower()] = key
            #                 type_map[str(key).strip()] = key
            #
            #             def map_account_type(val):
            #                 if pd.isna(val) or val is None or str(val).strip() == "":
            #                     return None
            ...
            #                 return type_map.get(val_str, type_map.get(val_str.lower(), val_str))
            #
            #             insert_data['account_type'] = insert_data['account_type'].apply(map_account_type)
            #         except Exception as e:
            #             _logger.warning(f"Could not map account_type: {e}")
            # =========================================================
                    ...
                    for field_name in final_insert_columns:
                        ...
                        if field_obj and getattr(field_obj, "translate", False):
                            ...
                            if pd.isna(raw_value) or raw_value in (None, '', 'nan', 'None'):
                                ...
                                # If field is also a selection type, convert display label -> internal key
                                # BEFORE wrapping as JSONB. e.g. 'Expenses' -> 'expense_other'
                                if getattr(field_obj, "type", None) == "selection":
                                    try:
                                        ...
                                        sel_map = {}
                                        for _k, _l in (sel or []):
                                            sel_map[str(_l).strip().lower()] = str(_k)
                                            sel_map[str(_k).strip().lower()] = str(_k)
                                        actual_value = sel_map.get(actual_value.lower(), actual_value)
                                    except Exception as _sel_err:
                                        _logger.warning("Selection label->key failed for %s: %s", field_name, _sel_err)
                                values.append(json.dumps({default_lang: actual_value}))
                            continue
                        ...
                        try:
                            if ftype == "selection":
                                ...
                                try:
                                    ...
                                    _logger.warning("sel map failed for %s: %s", field_name, _sel_err)
