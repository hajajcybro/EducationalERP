...
class EducationFeeType(models.Model):
    """Creates the model education.fee.type"""
    _name = 'education.fee.type'
    _description = 'Education Fee Type of the structure line'
    ...
    payment_type = fields.Selection([
        ...
        help='Payment type describe how much a payment effective. Like, bus '
             'fee per month is 30 dollar, sports fee per year is 40 dollar, '
             'etc')
    interval = fields.Char(string='Payment Interval',
                           help='Interval describe the payment mode of the fee.'
                                ...
                                'paid in each month.Yearly means the payment '
                                'paid only one time uin year.')
    category_id = fields.Many2one('education.fee.category', string='Category',
                                  required=True,
                                  default=lambda self: self.env[
                                      'education.fee.category'].search([],
                                                                       limit=1),
                                  help='Category of Education fee type')

    @api.model
    def create(self, vals):
        """Supering the already existing create method"""
