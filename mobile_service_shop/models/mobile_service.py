    ...
    def unlink(self):
        ...
        for service in self:
            if service.service_state != 'draft':
                raise UserError(
                    ('You cannot delete an assigned service request'))
        ...
        for order in self.product_order_line:
            if not order.stock_number:
                ...
                if order.product_uom_qty > order.qty_stock_move:
                    ...
                    raise UserError(
                        ('Used quantity is less than quantity stock move posted.'))
        if all_have_stock_number:
            raise UserError(
                ('All products have stock moves. No stock picking will be created.'))
        if flag == 0:
            raise UserError(('Nothing to post stock move.'))
        pick = {
