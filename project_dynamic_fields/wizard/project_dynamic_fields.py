...
class ProjectDynamicFields(models.TransientModel):
    ...
    _description = 'Project Dynamic Fields'
    ...
    fields_types = [
        ...
        'many2one', 'one2many', 'many2many', 'date', 'datetime', 'binary', 'html', 'monetary'
    ]
    ...
    def get_possible_field_types(self):
        ...
        field_list = sorted((f, f) for f in self.fields_types)
        for excluded in ['one2many', 'reference', 'properties', 'properties_definition']:
            field_list = [f for f in field_list if f[0] != excluded]
        return field_list
    ...
    def action_create_fields(self):
        ...
        <data>
            <field name="{self.position_field_id.name}" position="{self.position}">
                <field name="{self.name}" {'widget="' + self.widget_id.name + '"' if self.widget_id else ''}/>
            </field>
        </data>"""
    ...
    position_field_id = fields.Many2one('ir.model.fields', string='Existing Field Name',
    ...
    model_id = fields.Many2one('ir.model', string='Model', required=True,
                               ...
                               domain=_set_default)
    ref_model_id = fields.Many2one('ir.model', string='Reference Model', index=True,
                                   help="This field is used to specify the "
                                        "model that the dynamic field will be "
                                        "associated with.")
    selection_field = fields.Char(string="Selection",
                                  help='Enter selection value')
    rel_field_id = fields.Many2one('ir.model.fields', string='linked Related Field',
                                   help='Related field.')
    ...
    ttype = fields.Char(
        string='Technical Field Type',
        compute='_compute_ttype',
        store=False,
        help="Technical field type used when creating dynamic field."
    )
    ...
    def unlink(self):
        ...
        self = self.with_context(project_dynamic_field_deleted=True)
        if self.form_view_id:
            self.form_view_id.active = False
            views = self.mapped('form_view_id').filtered(lambda v: v.exists())
            for field in self:
        ...
        result=super().unlink()
        if self._context.get('project_dynamic_field_deleted'):
